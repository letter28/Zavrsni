...
def data_catcher():
    ...
    try:
        ...
        if int(ACpower[0]) == 0:
            ...
            c = db.cursor()
            insert_data = """INSERT INTO elektrana VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
            c.execute(insert_data, (my_date, my_time, int(DCpower[0]), int(ACpower[0]), float(eff[0]), int(MaxP[0]), float(f_mreze[0]),
